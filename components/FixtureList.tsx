...
export const FixtureList: React.FC<FixtureListProps> = ({ fixtures, filter = 'all', liveScores = {}, groups, pairings, useLocalTime, highlightedMatchId, overrides }) => {
    ...
    // AUTOMATISATION : Inclusion de TOUTES les nations possibles (Qualifiées + Barragistes)
    const allTeams = useMemo(() => {
        const teamsSet = new Set<string>();
        
        // 1. Nations déjà dans les Groupes (Constantes)
        ...
        // 2. Nations Candidates aux Playoffs (Nouveauté)
        Object.values(playoffCandidates).flat().forEach(t => {
            teamsSet.add(t);
        });

        // 3. Nations résolues dans les fixtures actuelles
        fixtures.forEach(f => {
    ...
    const getTimeContextStyles = (timeStr: string) => {
        ...
        if (minutes >= 420 && minutes < 1200) return { icon: "☀️", label: "Day", className: "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-800/30" };
        if (minutes >= 1200 && minutes < 1410) return { icon: "💎", label: "Prime", className: "bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 border-cyan-200 dark:border-cyan-800/30 shadow-[0_0_10px_rgba(34,211,238,0.15)]" };
        return { icon: "💤", label: "Night", className: "bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800/30" };
    };
